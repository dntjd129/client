...
const UploadThumbnail = ({ onFileChange }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      formData.append("thumbnail", selectedFile); // 필드 이름을 "thumbnail"로 설정
    ...
    <S.ImageUploadWrapper>
      ...
      <S.ImageBtnWrapper>
        <div style={{ position: "relative" }}>
          ...
          <S.FileTypeLeftInput type="file" onChange={handleFileChange} />
        </div>
        <AdminButtonGrey onClick={handleUpload}>썸네일 업로드</AdminButtonGrey>
      </S.ImageBtnWrapper>
    </S.ImageUploadWrapper>
  );
};
